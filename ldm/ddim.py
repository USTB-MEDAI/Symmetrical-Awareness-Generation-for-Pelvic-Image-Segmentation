...
from .ddpm_utils import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like
...
class DDIMSampler(object):
        ...
        **kwargs,
        ...
        if conditioning is not None:
            if isinstance(conditioning, dict):
                ...
                # conditioning[first_key] = conditioning[first_key][:1]  # 只保留第一个 batch
                # cbs = 1  # 更新 cbs 为 1
                # print(f"1Using conditioning of shape {conditioning[first_key].shape} for the first batch")
                if cbs != batch_size:
                    print(f"Warning: Got {cbs} conditionings but batch-size is {batch_size}")
            else:
                if conditioning.shape[0] != batch_size:
                    # conditioning = conditioning[:1]  # 只保留第一个 batch
                    # print(f"2Using conditioning of shape {conditioning.shape} for the first batch")
    ...
    def p_sample_ddim(
        ...
        _, *_, device = *x.shape, x.device
        b = 1

        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0:
            # print("Using conditional guidance!!!!!!!")
            # print(f"x shape {x.shape}, t shape {t.shape}, c shape {c.shape}")
        ...
        # print(f"x shape {x.shape}, e_t shape {e_t.shape}, a_t shape {a_t.shape}, a_prev shape {a_prev.shape}, sigma_t shape {sigma_t.shape}, sqrt_one_minus_at shape {sqrt_one_minus_at.shape}")
