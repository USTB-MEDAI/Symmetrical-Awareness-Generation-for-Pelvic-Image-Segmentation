...
class LPIPSWithDiscriminator(nn.Module):
    ...
    def forward(
        ...
        if self.perceptual_weight > 0: #没有执行
            p_loss = self.perceptual_loss(inputs.contiguous(), reconstructions.contiguous())
            rec_loss = rec_loss + self.perceptual_weight * p_loss
        if self.highlow_weight > 0: #没有执行
        ...
        nll_loss = rec_loss / torch.exp(self.logvar) + self.logvar  #nll_loss = rec_loss
        weighted_nll_loss = nll_loss
        if weights is not None:  #没有执行
        ...
        if optimizer_idx == 0:
            # generator update
            if cond is None:  #没有执行
                assert not self.disc_conditional
                logits_fake = self.discriminator(reconstructions.contiguous())
            else:
                assert self.disc_conditional
                logits_fake = self.discriminator(torch.cat((reconstructions.contiguous(), cond), dim=1))
            g_loss = -torch.mean(logits_fake)  #生成器的损失函数

            if self.disc_factor > 0.0: #执行
                try:
                    d_weight = self.calculate_adaptive_weight(nll_loss, g_loss, last_layer=last_layer) #计算自适应权重
                except RuntimeError:
