...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        x = torch.nn.functional.interpolate(x, (x.shape[2] * 2, x.shape[3] * 2, x.shape[4] * 2), mode="nearest")
        if self.with_conv:
            x = self.conv(x)
        return x
...
class Downsample(nn.Module):
    def __init__(self, in_channels, with_conv):
        ...
        if self.with_conv:
            ...
            self.conv = torch.nn.Conv3d(in_channels, in_channels, kernel_size=3, stride=(2, 2, 2), padding=1)
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False, dropout, temb_channels=512):
        ...
        if temb_channels > 0:
            self.temb_proj = torch.nn.AdaptiveAvgPool2d(temb_channels, out_channels)
        ...
        *,
        ...
        if self.use_timestep:
            ...
            self.temb.dense = nn.ModuleList(
                [
                    torch.nn.AdaptiveAvgPool2d(self.ch, self.temb_ch),
                    torch.nn.AdaptiveAvgPool2d(self.temb_ch, self.temb_ch),
                ]
            )
...
class Encoder(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
        ...
        self.down = nn.ModuleList()
        for i_level in range(self.num_resolutions):
            print(f"level {i_level}, in_ch_mult {in_ch_mult[i_level]}")
            block = nn.ModuleList()
            # attn = nn.ModuleList()
            block_in = ch * in_ch_mult[i_level]
            print(f"block_in {block_in}")
            block_out = ch * ch_mult[i_level]
            for i_block in range(self.num_res_blocks):
                block.append(
                    ResnetBlock(in_channels=block_in, out_channels=block_out, temb_channels=self.temb_ch, dropout=dropout)
