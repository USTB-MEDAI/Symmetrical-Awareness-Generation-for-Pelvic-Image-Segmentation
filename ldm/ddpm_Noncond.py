...
class DDPM(pl.LightningModule):
    ...
    def q_sample(self, x_start, t, noise=None):  #前向扩散过程
    ...
    def validation_step(self, batch, batch_idx):
        print("DDPM Validation step")
        _, loss_dict_no_ema = self.shared_step(batch)
        with self.ema_scope():
            _, loss_dict_ema = self.shared_step(batch)
            print(f"loss_dict_ema: {loss_dict_ema}")
...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        if self.conditional_key is not None:
            ...
            if self.high_low_mode:
                ...
                cond_cat = []
                if 1 in self.cond_nums and cond1 is not None:
                    cond1 = cond1.to(self.device)
                    encoder_posterior = self.encode_cond_stage(cond1)
                    cond1 = self.get_first_stage_encoding(encoder_posterior).detach()
                    cond_cat.append(cond1)
                # Repeat for cond2 and cond3 if needed and not None
    ...
    def encode_cond_stage(self, x):
        return self.cond_stage_model.encode(x)

    def shared_step(self, batch, **kwargs):
        print("LDM shared_step")
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        print(f"LDM x_start: {x_start.shape}")
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)  # * x -> x_t (noisey)
        print(f"LDM x_noisy1: {x_noisy.shape}")
    ...
    def p_mean_variance(
        ...
        t_in = t
        print(f"LDM p_mean_variance x: {x.shape}")
    ...
    def p_sample_loop(
        ...
        print("LDM p_sample_loop shape: {shape}")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.current_epoch % 10 == 0 and batch_idx == 1:
            ...
            if self.current_epoch % 200 == 0 and batch_idx == 1:
                ...
                print(f"ddim val shape :{shape}")
                cond_z, _ = ddim_sampler.sample(50, batch_size=1, shape=shape, conditioning=cond, verbose=False)
    ...
    def img_saver(self, img, post_fix, i_type=".nii", meta_data=None, **kwargs):
        """
        ...
        assert post_fix in ["origin_x", "ae_rec", "label","xray_left", "xray_right", "rec"], "unsupported post_fix"
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.conditional_key is None:
            print("No conditioning key specified, using unconditional diffusion model")
            _, c, x, x_rec = self.get_input(
                batch, self.first_stage_key, return_first_stage_outputs=True, return_original_cond=False
            )
        else:
            print(f"Conditioning key: {self.conditional_key}")
            if 1 in self.cond_nums:
        ...
        self.img_saver(x, "origin_x", meta_data=meta_data)
        self.img_saver(x_rec, "ae_rec", meta_data=meta_data)
        if self.conditional_key is None:
            pass
        else:
            if 1 in self.cond_nums:
                self.img_saver(origin_cond, "label", i_type=".nii", meta_data=cond1_meta_data)
            if 2 in self.cond_nums:
        ...
        self.img_saver(reconstructions, "rec", meta_data=meta_data)
