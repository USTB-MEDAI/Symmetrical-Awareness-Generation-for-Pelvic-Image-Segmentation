...
import sys
import os
# 将 ldm 目录添加到 Python 路径中
sys.path.append(os.path.join('/home/syz/Xray-Diffsuion'))
sys.path.append(os.path.join('/home/syz/Xray-Diffsuion/ldm'))
print(sys.path)

# from autoencoderkl.quantize import VectorQuantizer2 as VectorQuantizer
from ldm.autoencoderkl.model import Encoder
...
class vit_encoder_b(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.model = AutoEncoderViT(
                z_channels=4,
                resolution=128,
                in_channels=3,  #这里的in_channels
                out_ch=3,
                ch=32,
                ch_mult=[1, 2, 4, 4],
                num_res_blocks=2,
...
def load_weight_for_vit_encoder(pretrained, path):
    weight = None
...
class AutoEncoderViT(pl.LightningModule):
    def __init__(self, 
        ...
        self.quant_conv = torch.nn.Conv3d(2 * self.z_channels, 2 * self.embed_dim, 1)
    ...
    def forward(self, input, sample_posterior=True):
        ...
        if sample_posterior:
            ...
            z = posterior.mode()
        print(f"z shape: {z.shape}") # [b, 4, 16, 16, 16]
...
if __name__ == "__main__":
    ...
    # transunet = TransUNet(in_channels=1, out_channels=96, class_num=8)
